...
description: Upload documents to B-BAK to prepare them for signing. Covers supported formats, file size limits, and upload methods.
---
...
<Callout type="warn">
  B-BAK does not support password-protected or encrypted PDF files. Remove encryption before
  uploading.
</Callout>

## Supported Formats

B-BAK accepts two file formats:
...
<Callout type="info">
  ...
  B-BAK. Self-hosted instances must
...
![B-BAK Documents dashboard](/document-signing/documenso-documents-dashboard.webp)
...
<Steps>
  <Step>
    ...
    Open your Documents dashboard at [b-bak.de](https://b-bak.de).
...
If you do upload a `.docx` file, B-BAK converts it to PDF before adding it to the envelope. The original `.docx` is discarded. Only the converted PDF is stored, signed, and downloaded.
...
- **Form controls are flattened.** Word content controls (drop-downs, date pickers, checkboxes) become static text or graphics. Use B-BAK fields for anything that needs to be filled in.
- **Fonts not installed on the server fall back to substitutes.** On B-BAK, common fonts (Calibri, Arial, Times New Roman, etc.) are installed. On self-hosted instances, font fidelity depends on the operator's setup.
...
1. **Processing**: B-BAK validates the PDF and extracts page information
...
<Accordion title="Only PDF and DOCX files are allowed">
  B-BAK accepts PDF and DOCX. For other formats (`.doc`, `.odt`, `.rtf`, etc.), export to PDF
