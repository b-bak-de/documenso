...
When someone uses a direct link, B-BAK creates a new document from the template with that person as the signer.
...
```html
<iframe
  src="https://b-bak.de/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://b-bak.de/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://b-bak.de/d/${token}`;
}
```
...
```
https://b-bak.de/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://b-bak.de/d/${token}?${params.toString()}`;
}
...
```
https://b-bak.de/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a B-BAK account     |
...
Enterprise plans support white-label embedding with the "Powered by B-BAK" badge removed. Contact info@b-bak.de for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://b-bak.de/embed/direct/${token}?${params}`;
}
