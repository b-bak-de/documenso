...
Verifying webhook requests ensures that incoming payloads originate from B-BAK and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How B-BAK Signs Webhooks

When you configure a webhook with a secret, B-BAK includes that secret in every webhook request via the `X-Documenso-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in B-BAK |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in B-BAK             |
...
3. Update the webhook configuration in B-BAK
