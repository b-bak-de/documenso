...
1. You configure a webhook URL in B-BAK
2. When an event occurs, B-BAK sends an HTTP POST to your URL
3. Your application processes the event and responds with 200 OK

B-BAK supports webhook events for the full document lifecycle (created, sent, opened, signed, completed, rejected, cancelled), recipient-level events (recipient completed, reminder sent, recipient expired), and template events (created, updated, deleted, used).
