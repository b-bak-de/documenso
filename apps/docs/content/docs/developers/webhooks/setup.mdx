...
Webhooks are HTTP callbacks triggered by specific events in B-BAK. When an event occurs (such as a document being signed), B-BAK sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in B-BAK, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
Always respond with a `2xx` status within 10 seconds. B-BAK will retry failed deliveries according to the configured background-job provider.
</Callout>

## Configuring Webhooks in B-BAK via the Dashboard
...
<Callout type="warn">
B-BAK performs a best-effort check that rejects webhook URLs which use or resolve to private
...
B-BAK provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
B-BAK will attempt to deliver the same payload again
</Step>
</Steps>
...
Only the individual delivery (`WebhookCall`) record is marked as failed. B-BAK does not automatically disable the webhook or apply a circuit breaker, so future matching events continue to be delivered. After automatic attempts are exhausted, you can manually resend a failed delivery from the dashboard.
