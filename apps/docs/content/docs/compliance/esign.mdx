...
## How B-BAK Supports Compliance

B-BAK provides features that support compliance with e-signature laws across jurisdictions:
...
B-BAK maintains an audit trail for each document, recording:
...
## What B-BAK Does NOT Provide

B-BAK supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
