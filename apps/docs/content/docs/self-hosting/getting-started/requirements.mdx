---
title: Requirements
description: System requirements and prerequisites for self-hosting B-BAK.
---
...
B-BAK requires the following items and external services:
...
<Callout type="error">
  B-BAK does not ship with a signing certificate. Without one, the application starts normally
...
B-BAK uses PostgreSQL for all data storage including documents, users, and audit logs. You cannot use MySQL, SQLite, or other databases.

B-BAK requires two connection strings:
...
B-BAK sends transactional emails for:
...
For production deployments, place B-BAK behind a reverse proxy such as:
...
The reverse proxy handles SSL/TLS termination and forwards requests to B-BAK on port 3000.
...
By default, B-BAK stores documents in the PostgreSQL database. For production deployments with significant document volume, use S3-compatible storage:
...
B-BAK processes background jobs (email delivery, document processing) using a PostgreSQL-based queue by default. No additional services are required: the job queue is built into the application and uses your existing database.

For production deployments that need higher throughput or more reliable job processing, B-BAK supports [BullMQ](https://docs.bullmq.io/) as an alternative provider. BullMQ requires a **Redis** instance (v6.2+). Set `NEXT_PRIVATE_JOBS_PROVIDER=bullmq` and configure `NEXT_PRIVATE_REDIS_URL`.
...
- [ ] Domain name for your B-BAK instance
...
- [Quick Start](/docs/self-hosting/getting-started/quick-start) - Deploy B-BAK with Docker in 5 minutes
