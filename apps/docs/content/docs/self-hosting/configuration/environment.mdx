...
description: Complete reference for all environment variables used to configure a self-hosted B-BAK instance.
---

## Required Variables

These variables must be set for B-BAK to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your B-BAK instance (e.g., `https://sign.example.com`)                        |
...
B-BAK requires PostgreSQL 14 or higher.
...
Before delivering a webhook, B-BAK checks whether the target resolves to a
...
B-BAK supports multiple email transports for sending notifications.
...
B-BAK can store documents in the database or S3-compatible storage.
...
B-BAK requires a certificate to digitally sign documents.
...
These flags are opt-in: when none are set, signin behaviour is unchanged from a stock B-BAK instance.
...
B-BAK can use Google Vertex AI for recipient and field detection.
...
B-BAK can accept `.docx` uploads by sending them to a [Gotenberg](https://gotenberg.dev) service that converts them to PDF. When `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL` is unset, DOCX uploads are rejected and only PDFs are accepted.
...
B-BAK supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
