...
S3 storage is recommended for production deployments. B-BAK supports AWS S3 and any S3-compatible storage service.
...
Using Docker Compose with B-BAK:
...
B-BAK works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
Azure Blob Storage is supported as a native transport (not S3-compatible). B-BAK uses the official `@azure/storage-blob` SDK and signs SAS URLs with the Storage Account key for browser uploads and downloads.
...
- An S3 bucket configured for B-BAK
...
B-BAK does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, B-BAK can read from both backends. The `DocumentData.type` field indicates where each document is stored.
