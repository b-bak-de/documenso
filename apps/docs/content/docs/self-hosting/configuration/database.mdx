...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your B-BAK deployment.
---
...
B-BAK requires **PostgreSQL 14 or later**. No other databases are supported.
...
B-BAK uses two database connection variables:
...
B-BAK automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, B-BAK will use them automatically.
...
B-BAK uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading B-BAK.

### Automatic Migrations

When running B-BAK via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for B-BAK workloads:
...
B-BAK includes necessary indexes by default. Additional indexes may help for:
