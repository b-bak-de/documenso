...
B-BAK supports four email transport methods:
...
SMTP is the most common email transport. B-BAK supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for B-BAK to use.
...
B-BAK uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the B-BAK logo
...
Configure B-BAK to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="B-BAK Test"
```
...
For MailChannels, configure DKIM directly in B-BAK using the environment variables described above.
