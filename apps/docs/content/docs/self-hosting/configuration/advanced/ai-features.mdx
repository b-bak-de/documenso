...
description: Configure Google Vertex AI so B-BAK can detect recipients and fields automatically.
---
...
This guide covers how to enable the AI recipient and field detection features when you self-host B-BAK.
...
- B-BAK version that includes the AI detection feature and the corresponding database migration.
...
<Callout type="info">
  ...
  If you omit the location, B-BAK uses `global`. Not all models are available in every region;
  if a model is unavailable, switch to a supported region.
</Callout>

## Deploy with the Published Container

- Use the official B-BAK image (DockerHub or GHCR) and supply the Vertex env vars above.
