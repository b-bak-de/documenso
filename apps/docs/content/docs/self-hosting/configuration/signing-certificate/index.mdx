...
description: Configure the X.509 signing certificate used for digital signatures in self-hosted B-BAK.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

When a document is completed in B-BAK, it is digitally signed using an X.509 certificate. This cryptographic signature provides:
...
Self-hosted B-BAK instances require a signing certificate. You can generate a self-signed certificate or purchase one from a Certificate Authority (CA).
...
<Cards>
  ...
  <Card
    ...
    description="Use the Trusted Signatures cloud service for Adobe-trusted PDF signatures."
...
For organisations requiring hardware-based key protection, B-BAK supports Google Cloud HSM. This provides:
...
For Advanced and Qualified Electronic Signatures under eIDAS, B-BAK integrates with third-party Trust Service Providers via the Cloud Signature Consortium API. Each recipient authenticates directly with the TSP, which holds the private key and issues the signature.
...
- Instance-wide setting; one CSC provider per B-BAK install
...
Set `NEXT_PRIVATE_SIGNING_TRANSPORT=trusted-signatures` to use Trusted Signatures instead of a local certificate or HSM. Configure `NEXT_PRIVATE_SIGNING_TS_API_KEY_ID` and the hexadecimal `NEXT_PRIVATE_SIGNING_TS_API_KEY`. Set `NEXT_PRIVATE_SIGNING_TS_TIMESTAMP=true` to request a timestamp from the service.
