...
- An active Enterprise license key — contact [sales](https://documen.so/enterprise) to set up an
  ...
  - A running self-hosted B-BAK instance that you're able to restart
...
If you're running B-BAK directly (not in a container), add the variable to your `.env` file:
...
On startup, B-BAK validates the key against the license server and caches the result locally for future startups, so a brief license-server outage won't lock you out.
