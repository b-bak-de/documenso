---
title: Docker
description: Run B-BAK as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
The B-BAK image is available on both DockerHub and GitHub Container Registry:
...
These variables must be set for B-BAK to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where B-BAK is accessible (e.g., `https://sign.example.com`)      | -                             |
...
B-BAK requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="B-BAK" \
...
B-BAK provides health check endpoints for monitoring:
...
To update B-BAK to a new version:
...
The B-BAK container is stateless. All persistent data is stored in the PostgreSQL database.
