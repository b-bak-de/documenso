---
title: Railway
description: Deploy B-BAK on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
The fastest way to deploy B-BAK on Railway is using the official template:
...
Click the URL in the Railway dashboard to access your B-BAK instance.
...
If you prefer more control over the setup, deploy B-BAK manually:
...
- Fork or select the [B-BAK repository](https://github.com/b-bak-de/documenso)
...
Click on the B-BAK service and go to the "Variables" tab. Add the following variables:
...
NEXT_PRIVATE_SMTP_FROM_NAME=B-BAK
...
B-BAK runs database migrations automatically on startup.
...
Railway provides several options for scaling your B-BAK deployment:
...
<Steps>
<Step>
### Open your B-BAK service

In the Railway dashboard, select the B-BAK service.
...
In the Railway dashboard, select your B-BAK service and open "Settings".
...
<Callout type="warn">
B-BAK stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for B-BAK:
...
B-BAK requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

B-BAK generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your B-BAK service variables, set:
...
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/b-bak-de/documenso)

### Koyeb

<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/b-bak-de/documenso&branch=main&name=bbak-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img
